    ...
    return (
      <AuthLayout>
        <Card className="w-full max-w-md text-center">
          <p className="text-sm text-rose-400 font-medium">Không tìm thấy cửa hàng "{slug}".</p>
          <button
            type="button"
            onClick={() => redirectToDiscovery('/owner/login')}
            className="mt-4 inline-block text-sm font-bold text-purple-400 hover:text-purple-300 transition-colors"
          >
            ← Thử địa chỉ khác
          </button>
        </Card>
    ...
    <AuthLayout>
      <Card className="w-full max-w-md">
        <div className="flex flex-col items-center gap-2 text-center">
          ...
          <p className="font-['JetBrains_Mono'] text-xs text-purple-300/80">
        ...
        <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label htmlFor="password" className="text-sm font-semibold font-['Plus_Jakarta_Sans',sans-serif] text-[#C5C0CD]">Mật khẩu</label>
              <button
                type="button"
          ...
          {error && !pendingActivation && <p className="text-sm text-rose-400 font-medium">{error}</p>}
          {pendingActivation && (
            <Callout
              tone="warning"
              action={
          ...
          <Button type="submit" size="lg" className="w-full justify-center mt-2" disabled={mutation.isPending}>
        ...
        <p className="mt-8 text-center text-sm font-['Plus_Jakarta_Sans',sans-serif] text-[#A7A1AD]">
          <button
            type="button"
            onClick={() => redirectToDiscovery('/owner/login')}
            className="hover:underline text-[#A7A1AD] hover:text-white transition-colors"
          >

...
export default function RequireManagerAuth() {
    ...
    return (
      <div className="flex min-h-dvh items-center justify-center bg-stone-50 dark:bg-zinc-950">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Đang kiểm tra phiên làm việc Manager...</p>
        </div>
      </div>
    );
  }
  ...
  if (user && !user.roles.includes('BRANCH_MANAGER') && !user.roles.includes('OWNER')) {

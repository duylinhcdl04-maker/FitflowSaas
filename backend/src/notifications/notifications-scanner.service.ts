...
export class NotificationsScannerService {
    ...
    try {
      ...
      for (const branch of branches) {
        try {
          ...
          this.logger.error(
            `Notification scan failed for branch ${branch.id}`,
            err as Error,
          );
        }
      }
      ...
      this.logger.warn(
        `[NotificationsScanner] Bỏ qua lượt quét do chưa kết nối được DB (Neon cold start): ${(err as Error).message}`,
      );
    }
  }
  ...
  private async scanBranch(
    ...
    const [pendingPayments, expiringMemberships, atRiskCustomers, recipients] =
      await Promise.all([
    ...
    const pendingPaymentsItems: NotificationDetailItem[] = pendingPayments.map(
      (p) => ({
        id: p.id,
    ...
    const expiringMembershipsItems: NotificationDetailItem[] =
    ...
    const atRiskItems: NotificationDetailItem[] = atRiskCustomers.map((c) => ({
      ...
      startDate:
        c.memberships[0]?.start_date?.toISOString() ??
  ...
  private async syncOne(opts: {
    ...
    if (opts.count === 0) {
      await this.notifications.resolveByDedupe(
        opts.tenantId,
        opts.eventCode,
        dedupeSuffix,
